...
--tbl_user table에서는 userid는 중복되지 않으며 tbl_user에서는 1개만 존재하는 값이다.

--tbl_hobby table에서 userid는 여러개의 데이터가 있을 수 있으며 userid 칼럼을 기준으로 tbl_user와 1:다의 관계를 갖는다.

--tbl_user와 tbl_hobby의 1:다의 관계를 FK로 설정한다.

--PK키를 설정할 때는 다(多)의 위치에 있는 table을 기준으로 ALTER를 실행한다.
...
--USER에 userid가 없음연 HOBBY에는 절대 있을 수 없다.(있어서는 안됌)

--USER에 없는 userid를 HOBBY에 추가하려고 하면, 오류가 발생한다.
--만약에 HOBBY에 있는 userid를 USER에서 삭제하려고 시도하면 FK의 조건에 위배되어 삭제 할 수 없다.
