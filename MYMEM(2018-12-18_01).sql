...
--지금 현재 tbl_iolist는 상품이름과 거래처명, 거래처대표명을 직접 가지고있기 때문에 만약 상품이름이나 거래처명 거래처대표명을 변경
--삭제 하려고 하면 다수의 데이터 변경이 필요한데 이 때 다수의 데이터를 변경하는 것은 DB 사용환경, 규칙 입장에서는 매우 바람직 하지않다.
--그래서 현재의 tbl_iolist 테이블을 분리 작업하여 데이터변경이 다수 발생하지 않도록 조치를 취해야한다. 이를 이론적으로 제2 정규화(2NF)라고 한다.

--1. 거래처명과 거래처대표명을 분리
--가. tbl_iolist로 부터 거래처명(io_dname)과 거래처대표명(io_ceo) 묶어서 모아보자
--거래처명과 거래처대표명을 확인
SELECT io_dname, io_ceo FROM tbl_iolist ORDER BY io_dname;

--거래처명과 거래처대표명을 묶어서 확인
--tbl_iolist에서 거래처명과 거래처대표명을 한개씩만 추출한것
...
--마. iolist테이블에서 거래처명, 대표장명을 제거하고 거래처 코드칼럼으로 대체
...
--e. iolist와 dept 테이블을 JOIN해서 거래처명과 대표자명을 같이 조회하는 SQL 명령
